...
export default function BranchStorekeeperReviewPage() {
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/stock-issuance/branch-storekeeper/requests`, {
        credentials: 'include',
      });
  ...
  const submitReview = async (request: BranchReviewRequest) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/stock-issuance/branch-storekeeper/review/${request.request_id}`, {
        method: 'POST',
        credentials: 'include',
