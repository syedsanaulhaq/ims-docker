...
export default function ReceivingReportPage() {
  ...
  const fetchPOAndDeliveries = async () => {
    try {
      ...
      const poResponse = await fetch(`/api/purchase-orders/${id}`);
      if (!poResponse.ok) throw new Error('Failed to fetch purchase order');
      const poData = await poResponse.json();
      setPo(poData);

      // Fetch all deliveries for this PO
      const deliveriesResponse = await fetch(`/api/deliveries/by-po/${id}`);
      ...
      const itemsPromises = deliveriesData.map(async (delivery: Delivery) => {
        const itemsResponse = await fetch(`/api/deliveries/${delivery.id}/items`);
      ...
      const serialPromises = deliveriesData.map(async (delivery: Delivery) => {
        const serialResponse = await fetch(`/api/deliveries/${delivery.id}/serial-numbers`);
