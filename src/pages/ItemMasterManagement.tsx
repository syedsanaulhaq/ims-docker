...
const ItemMasterManagement = () => {
  ...
  const loadItems = async (includeDeleted = false) => {
    try {
      ...
      const url = includeDeleted 
        ? `${import.meta.env.VITE_API_URL}/api/item-masters?includeDeleted=true`
        : `${import.meta.env.VITE_API_URL}/api/item-masters`;
  ...
  const handleDeleteItem = async (itemId: number, itemName: string) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/item-masters/${itemId}`, {
        method: 'DELETE'
      });
  ...
  const handleRestoreItem = async (itemId: number, itemName: string) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/item-masters/${itemId}/restore`, {
        method: 'POST'
      });
  ...
  return (
    <div className="p-8">
      ...
      <div className="mt-6 p-4 bg-gray-100 rounded text-sm">
        <strong>Debug:</strong> 
        <div>API: ${import.meta.env.VITE_API_URL}/api/item-masters</div>
