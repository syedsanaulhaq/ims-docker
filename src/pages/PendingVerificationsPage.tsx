...
export const PendingVerificationsPage: React.FC = () => {
  ...
  const fetchPendingVerifications = async () => {
    try {
      ...
      const response = await fetch(`/api/inventory/my-verification-requests?userId=${encodeURIComponent(user.user_id)}`);
  ...
  const handleQuickAction = async (request: InventoryVerificationRequest, action: 'approve' | 'reject' | 'forward') => {
    ...
    try {
      const response = await fetch('/api/inventory/update-verification', {
        method: 'POST',
  ...
  const handleViewDetails = async (request: InventoryVerificationRequest) => {
    ...
    try {
      const skResponse = await fetch(`/api/ims/users?wing_id=${request.wing_id}&role=WING_STORE_KEEPER`);
  ...
  const handleSubmitVerification = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/inventory/forward-verification-to-storekeeper', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
