            ...
            userRequests.map(async (request: any) => {
              ...
              try {
                const approvalResponse = await fetch(
                  `${import.meta.env.VITE_API_URL}/api/approvals/request/${request.id}/items`,
                  { credentials: 'include' }
                );
              ...
              if (!approvalStatus) {
                try {
                  const raResponse = await fetch(
                    `${import.meta.env.VITE_API_URL}/api/approvals/request/${request.id}/status`,
                    { credentials: 'include' }
                  );
                  if (raResponse.ok) {
                  ...
                  {(['issued', 'dispatched', 'delivered'].includes(request.current_status) ||
                    ...
                    <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 mb-3 space-y-2">
                      ...
                      {request.delivery_proof_url && (
                        <a href={`${import.meta.env.VITE_API_URL}${request.delivery_proof_url}`}
