...
export const ItemGroupsManager: React.FC = () => {
  ...
  const loadGroups = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/item-groups`);
  ...
  const loadGroupItems = async (groupId: string) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/item-groups/${groupId}/items`);
      const data = await response.json();
  ...
  const handleDeleteGroup = async (groupId: string) => {
    if (confirm('Are you sure you want to delete this group? Associated items will be removed.')) {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/item-groups/${groupId}`, {
          method: 'DELETE'
        });
