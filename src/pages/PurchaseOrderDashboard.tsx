...
export default function PurchaseOrderDashboard() {
  ...
  const fetchPurchaseOrders = async () => {
    try {
      setLoading(true);
      let query = `${import.meta.env.VITE_API_URL}/api/purchase-orders`;
      ...
      const posWithStatus = await Promise.all(
        data.map(async (po: PurchaseOrder) => {
          if (po.status === 'finalized' || po.status === 'completed') {
            try {
              const statusResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/purchase-orders/${po.id}/delivery-status`);
  ...
  const handleDeletePO = async (id: number) => {
    ...
    try {
      setDeletingId(id);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/purchase-orders/${id}`, {
        method: 'DELETE'
      });
  ...
  const handleRestorePO = async (id: number) => {
    ...
    try {
      setRestoringId(id);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/purchase-orders/${id}/restore`, {
        method: 'POST'
      });
  ...
  const handleFinalizePO = async (id: number) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/purchase-orders/${id}/finalize`, {
        method: 'PUT',
