...
const StockQuantitiesPage: React.FC = () => {
  ...
  const loadQuantities = async () => {
    try {
      ...
      const response = await fetch('/api/inventory/stock-breakdown?show_zero_stock=true', {
        credentials: 'include',
        headers: {
  ...
  const loadItemDetails = async (item: StockQuantity) => {
    ...
    try {
      const response = await fetch(`/api/inventory/current-stock/${item.item_master_id}/history`, {
        credentials: 'include'
      });
