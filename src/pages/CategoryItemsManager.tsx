...
export const CategoryItemsManager: React.FC = () => {
  ...
  const loadAllItems = async () => {
    try {
      ...
      const itemsResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/item-masters`);
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/item-masters/${deleteItem.id}`, {
        method: 'DELETE'
      });
