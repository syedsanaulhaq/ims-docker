...
const EnhancedStockAcquisitionDashboard: React.FC = () => {
  ...
  const loadTenderItems = async (tenderId: string) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/stock-acquisition/items/${tenderId}`);
      if (response.ok) {
  ...
  const saveActualPrice = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/stock-acquisition/update-price/${editingItem.id}`, {
        method: 'PUT',
        headers: {
