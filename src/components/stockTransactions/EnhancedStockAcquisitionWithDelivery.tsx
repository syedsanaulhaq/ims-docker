...
const EnhancedStockAcquisitionWithDelivery: React.FC = () => {
  ...
  const loadTenderDetails = async (tenderId: string) => {
    try {
      ...
      const itemsResponse = await fetch(`/api/stock-acquisition/items/${tenderId}`);
      if (itemsResponse.ok) {
  ...
  const saveAllItemPrices = async () => {
    try {
      ...
      const response = await fetch('/api/stock-acquisition/update-multiple-prices', {
        method: 'PUT',
        headers: {
  ...
  const saveDeliveryInfo = async () => {
    ...
    try {
      const url = deliveryInfo.id 
        ? `/api/deliveries/${deliveryInfo.id}`
        : '/api/deliveries';
