  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        // Fetch vendors
        const vendorsRes = await fetch(`${import.meta.env.VITE_API_URL}/api/vendors`);
        ...
        // Fetch categories
        const catsRes = await fetch(`${import.meta.env.VITE_API_URL}/api/categories`);
        ...
        for (const cat of catsList) {
          const itemsRes = await fetch(`${import.meta.env.VITE_API_URL}/api/items-master?category_id=${cat.id}`);
          if (itemsRes.ok) {
  ...
  useEffect(() => {
    if (editingId && editingId !== 'new') {
      const fetchTender = async () => {
        try {
          const res = await fetch(`${import.meta.env.VITE_API_URL}/api/tenders/${editingId}`);
          if (res.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = editingId && editingId !== 'new'
        ? `${import.meta.env.VITE_API_URL}/api/tenders/${editingId}`
