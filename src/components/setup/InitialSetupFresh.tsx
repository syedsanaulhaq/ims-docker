...
const InitialSetupFresh: React.FC = () => {
  ...
  const fetchCurrentStock = async () => {
    try {
      ...
      let stockResponse;
      try {
        stockResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/inventory/current-stock-detailed`);
        if (!stockResponse.ok) {
  ...
  const saveUpdatedStock = async () => {
    try {
      ...
      let response;
      try {
        response = await fetch(`${import.meta.env.VITE_API_URL}/api/inventory/update-stock-quantities`, {
          method: 'POST',
          headers: {
        ...
        response = await fetch(`${import.meta.env.VITE_API_URL}/api/inventory/initial-setup`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
