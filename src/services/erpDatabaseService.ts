...
class ERPDatabaseService {
  ...
  async getActiveDecs(): Promise<DEC[]> {
    try {
      const response = await fetch('/api/decs');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  async getActiveUsers(): Promise<any[]> {
    try {
      const response = await fetch('/api/aspnet-users/active');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  async getFilteredUsers(officeId: number, wingId: number, branchId?: number | string): Promise<any[]> {
    try {
      let url = `/api/aspnet-users/filtered?officeId=${officeId}&wingId=${wingId}`;
